...
//        }
    ...
    fun saveToLog(logJsonString: String, context: Context?) {
        if (getFileValue(Constants.LOG_FILE_NAME, context) == null) {
            setFileValue(Constants.LOG_FILE_NAME, logJsonString, context)
        } else {
            appendFileValue(Constants.LOG_FILE_NAME, logJsonString, context)
        }
    }
}